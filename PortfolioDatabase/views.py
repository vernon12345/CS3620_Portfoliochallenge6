...
def update_portfolio(request,id):#contact function
    portfolio1 = portfolio.objects.get(id=id)
    form = HobbyForm(request.POST or None,instance=portfolio1)
    if (form.is_valid()):
        form.save()
        return redirect('PortfolioDatabase:index')

    return render(request, 'portfolio/portfolio-form.html', {'form': form, 'portfolio1':portfolio1})

def delete_portfolio(request,id):#contact function
    portfolio1 = portfolio.objects.get(id=id)
    if request.method == 'POST':
        portfolio1.delete()
        return redirect('PortfolioDatabase:index')

    return render(request, 'portfolio/portfolio-delete.html', {'hobby':portfolio1})
